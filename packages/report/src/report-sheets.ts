...
function complianceChecklistSpec(model: ProjectModel, sol: ProjectSolution): Spec {
  ...
  const rows: Row[] = [
    ...
    { req: `Demand applied over the most demanding area (${s.operatingHeads} heads${s.designAreaFt2 ? ` / ${u.areaU(s.designAreaFt2)}` : ""}${s.dryAreaIncreasePct ? ` · +${s.dryAreaIncreasePct}% dry` : ""}${s.slopeAreaIncreasePct ? ` · +${s.slopeAreaIncreasePct}% sloped` : ""}${s.qrAreaReductionPct ? ` · −${s.qrAreaReductionPct}% QR` : ""})`, status: "PASS", ref: s.dryAreaIncreasePct || s.slopeAreaIncreasePct || s.qrAreaReductionPct ? `${stdId} §19.3.3.2.8` : `${stdId} Fig.19.3.3.1.1` },
    ...(s.designDensityGpmFt2 && s.deliveredDensityGpmFt2 !== undefined ? [{ req: `Delivered density at the most remote sprinkler ≥ design (${u.densU(s.deliveredDensityGpmFt2)} ≥ ${u.densU(s.designDensityGpmFt2)})`, status: (s.deliveredDensityGpmFt2 >= s.designDensityGpmFt2 - 1e-4 ? "PASS" : "REVIEW") as Row["status"], ref: `${stdId} §19.2.1` }] : []),
